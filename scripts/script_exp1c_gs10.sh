...
if [ "$1" = "joint" ] || [ "$1" = "dmc" ]; then
        echo "Copy the experiment folders from exp1a for this approach."
elif [ "$1" = "icarl" ] || [ "$1" = "bic" ] || [ "$1" = "luci" ] || [ "$1" = "eeil" ]; then
        echo "Those approaches only support exemplars."
else
        PYTHONPATH=$SRC_DIR python3 -u $SRC_DIR/main_incremental.py --exp_name exp1c_gs10_seed$SEED \
