...
RESULTS_DIR="$PROJECT_DIR/results_emb_2"
if [ "$3" != "" ]; then
    RESULTS_DIR=$3
else
...
PYTHONPATH=$SRC_DIR python3 -u $SRC_DIR/main_incremental.py --exp_name align_triplet_${NUM_TASKS}_seed_${SEED} \
       --datasets cifar100 --num_tasks $NUM_TASKS --network resnet32 \
       --nepochs 200 --batch_size 300 --results_path $RESULTS_DIR \
       --lr 0.0001 --momentum 0.9 --weight_decay 5e-4 --lr_patience 15 --lr_factor 10 --lr_min 0.000001 \
       --approach emb_align --gpu $GPU --seed $SEED \
       --fix_bn --metric_loss align_uniform --stop_at_task 3 --m_per_class 30
