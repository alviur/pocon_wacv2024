...
class Appr(Learning_Appr):
    ...
    def train(self, t, trn_loader, val_loader):
        ...
        if t == 0:
            ...
            self.modelFB = SimSiam(
                models.__dict__['resnet18'],
                2048,
                self.p2_hid_dim,
                num_classes=self._num_classes,
                tbwriter=self._tbwriter,
                maxEpochs=self.nepochs,
                kd_method=self.kd_method,
                p2_hid_dim=self.p2_hid_dim,
                pred_like_p2=self.pred_like_p2,
                eval_nepochs=self.eval_nepochs,
                diff_lr=self.diff_lr,
                change_lr_scheduler=self.change_lr_scheduler,
            ...
            #path = '/home/agomezvi/simSiamLB2/modelsProj512/tiny/FT/' # FT
            #path = '/home/agomezvi/simSiamLB2/modelsProj512/tiny/EWC/'  # EWC
            #path = '/home/agomezvi/simSiamLB2/modelsProj512/tiny/P2/'  # P2
            #path = '/home/agomezvi/simSiamLB2/modelsProj512/tiny/FD/'  # FD
            path = '/home/agomezvi/simSiamLB2/modelsProj512/tiny/single/'  # single
            #path = '/home/agomezvi/simSiamLB5/models/P2/'
            #path = '/home/agomezvi/simSiamLB2/modelsBirds/FT/'# Birds FT
            #path = '/home/agomezvi/Desktop/simSiamCorrections/cvc-class-il/cifar100_noTrans_simsiam_simsiam_p2-10072021_150917_368433/'
            #modelDict = torch.load(path + 'task_0.ckpt', map_location='cuda:0')
            modelDict = torch.load(path + 'task_0.pth.tar', map_location='cuda:0')
            # self.modelFB.load_state_dict(modelDict['state_dict'])
            # print(modelDict)
            #self.modelFB = load_my_state_dict(self.modelFB, modelDict['simsiam'])
            self.modelFB = load_my_state_dictPara(self.modelFB, modelDict['state_dict'])
            ...
            #path = '/home/agomezvi/simSiamLB2/modelsProj512/tiny/EWC/'  # EWC
            #path = '/home/agomezvi/simSiamLB2/modelsProj512/tiny/P2/'  # P2
            #path = '/home/agomezvi/simSiamLB2/modelsProj512/tiny/P2_f/'  # P2_f
            path = '/home/agomezvi/simSiamLB2/modelsProj512/tiny/FD/'  # FD
            #path = '/home/agomezvi/simSiamLB2/tinyModels/'
            #path = '/home/agomezvi/simSiamLB5/models/P2/'
            #path = '/home/agomezvi/simSiamLB2/modelsBirds/FT/'  # Birds FT
            # path = '/home/agomezvi/Desktop/simSiamCorrections/cvc-class-il/cifar100_noTrans_simsiam_simsiam_p2-10072021_150917_368433/'
            modelDict = torch.load(path + 'model-task_'+str(t)+'.ckpt', map_location='cuda:0')
            #modelDict = torch.load(path + 'task_' + str(t) + '.pth.tar', map_location='cuda:0')
            # self.modelFB.load_state_dict(modelDict['state_dict'])
            # print(modelDict)
    ...
    def get_embeddings(self, t, trn_loader, val_loader):
        ...
        with override_dataset_transform(trn_loader.dataset, self.val_transforms) as _ds_train, \
            ...
            for img_1, y in _train_loader:
                ...
                self.embeddingsSave.append(torch.std(_x,dim=1).cpu().numpy())
                wandb.log({"STD(z)":torch.mean(torch.std(_x,dim=1))})
                _x = _x.detach()
...
class SimSiam(nn.Module):
    ...
    def __init__(self, base_encoder, dim, pred_dim, num_classes, tbwriter,
                 maxEpochs=800,
                 t=0,
                 kd_method='ft',
                 p2_hid_dim=None,
        ...
        self.tbwriter: SummaryWriter = tbwriter
        self.wandblog = False
        ...
        # lightly encoder includes modifications on first layer and maxpool 1 off
        ...
        self.projector = nn.Sequential(nn.Linear(512, 512, bias=False),
                                       nn.BatchNorm1d(512),
        ...
        # Lighltly
        '''self.projector = nn.Sequential(nn.Linear(512, 2048, bias=False),
                                       nn.BatchNorm1d(2048),
                                       nn.ReLU(inplace=True),  # hidden layer
        ...
        #                                nn.ReLU(inplace=True),  # second layer
        #                                self.encoder.fc,
        #                                nn.BatchNorm1d(dim))  # output layer
        ...
        self.predictor = nn.Sequential(nn.Linear(512, 256, bias=False),
                                       nn.BatchNorm1d(256),
                                       nn.ReLU(inplace=True),  # hidden layer
    ...
    def forward(self, x1, x2):
        ...
        Output:
            loss: contrastive loss + distillation loss if it was selected
        Notation:
            f1, f2: output of backbone
            p1, p2: output of projection
            z1, z2: output of predictors
        ...
        if self.t > 0 and self.kd == 'p2':
            ...
            lossKD = self.lamb[self.t - 1, 0] * (torch.dist(f1Old, f1))
    ...
    def configure_optimizers(self):
        ...
        if self.t < 1:
            ...
            params = [
                ...
                {'params': self.predictor.parameters(), 'lr': lr if self.diff_lr else 0.8*lr},
            ]
...
def load_my_state_dictPara(model, stateDictSaved):

    #own_state = model.state_dict()
    for name, param in  model.state_dict().items():
        if 'module.'+name not in stateDictSaved:
            continue
        # if isinstance(param, Parameter):
        #     # backwards compatibility for serialized parameters
        #     param = param.data
        print("loading", name)
        model.state_dict()[name].copy_(stateDictSaved['module.'+name])
