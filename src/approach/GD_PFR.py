...
class Appr(Learning_Appr):
    ...
    def __init__(
        ...
        if self.wandblog:
            wandb.init(project="Barlow Twins FB")
    ...
    def extra_parser(args):
        ...
        parser.add_argument("--kd_method", default="ft", type=str, choices=['ft', 'L2', 'EWC', 'PFR','EWC_p2','GD_PFR'])
        ...
        parser.add_argument("--lambdaExp", default=3.0, type=float)
    ...
    def get_data_loaders(self, trn_loader, val_loader, t):  # -> Replace _prepare_transformations
        ...
        if self.dataset2 == 'tiny':
            ...
            collate_fn = lightly.data.SimCLRCollateFunction(input_size=input_height, gaussian_blur=0., normalize=cifar_normalize,
        ...
        if self.kd_method == "EWC" or self.kd_method == "EWC_p2":
    ...
    def train(self, t, trn_loader, val_loader):
        ...
        if t == 0:
            ...
            self.modelFB = BarlowTwins(self.projectorArc,self.batch_size,self.lambd,self.change_lr_scheduler,
                                       self.nepochs,self.diff_lr,self.kd_method,self.lambdap2, self.lambdaExp)
        ...
        ## Retrospection
        if self.kd_method == "GD_PFR" and t>0:
            print("--------Retrospection phase-------")
            self.optimizer, self.scheduler = self.modelFB.configure_optimizers()
            ...
            self.modelFB.to(self.device)
            for epochs in range(self.ret_nepochs):
                self.train_loop(t, trn_loader, val_loader, epochs)
        ...
        for param in self.modelFB.oldProjector.parameters():
            param.requires_grad = False
        ...
        if epochs % 100 == 0:
            ...
            }, is_best=False, filename='task_' + str(
                t) + 'checkpoint_{:04d}.pth.tar'.format(epochs))




        if self.kd_method == "GD_PFR":
            # Create an expert model for next task
            # ResNet18
        ...
        # Copy P1 into P2
        # if t>0:
        #     self.modelFB.p2 = deepcopy(self.modelFB.predictor)
...
class BarlowTwins(nn.Module):
    def __init__(self, projectorArc,batch_size,lambd,change_lr_scheduler,maxEpochs,diff_lr,kd_method,lambdap2,lambdaExp):
        super().__init__()
        ### Barlow Twins params ###
        self.projectorArc = projectorArc
    ...
    def forward(self, x1, x2):
        ...
        if self.t > 0 and self.kd == 'GD_PFR' and self.retrospection:
            ...
            p2_2 = self.p2(f2)
            # Map to expert space
            p2_1exp = self.p2exp(f1)
            p2_2exp = self.p2exp(f2)

            lossKD = self.lambdap2 * (-(self.criterion(p2_1, f1Old.detach()).mean()
                                        + self.criterion(p2_2, f2Old.detach()).mean()) * 0.5)\
                     + self.lambdaExp * (-(self.criterion(p2_1exp, f1Expert.detach()).mean()
                                        + self.criterion(p2_2exp, f2Expert.detach()).mean()) * 0.5)
    ...
    def configure_optimizers(self):
        ...
        if self.t < 1:
            ...
            if self.kd == 'p2' or self.kd == 'p2_f' or self.kd == 'EWC_p2' or self.kd == 'GD_PFR':
                params.append({'params': self.expertBackbone.parameters()})
                params.append({'params': self.p2.parameters()})
                params.append({'params': self.p2exp.parameters()})
