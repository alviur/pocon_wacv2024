...
class Appr(Learning_Appr):
    ...
    def get_data_loaders(self, trn_loader, val_loader, t):  # -> Replace _prepare_transformations
        ...
        if self.dataset2 == 'tiny':
            ...
            imagenet_normalize = {'mean': [0.5071, 0.4866, 0.4409], 'std': [0.2009, 0.1984, 0.2023]}
    ...
    def train(self, t, trn_loader, val_loader,output_path):
        ...
        if t == 0:
            ...
            if self.sslModel == "BT":
                self.modelFB = BarlowTwins(self.projectorArc, self.batch_size, self.lambd, self.change_lr_scheduler,
                                           self.task1_nepochs, self.diff_lr, self.kd_method, self.lambdapRet, self.lambdaExp,
                                           self.lr, self.expProjSize, self.adaptSche, self.linearProj, self.linearProjRet,
                                           self.norm,self.expertArch,self.lamb)
        ...
        if t > 0:
            ...
            # for p1, p2 in zip(self.modelFB.oldBackbone.parameters(), self.modelFB.backbone.parameters()):
            #     if p1.data.ne(p2.data).sum() > 0:
            #         print("Different!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            # print("Equal!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")

            if t==0:
                adaRet_nepochs = 3*self.ret_nepochs
            else:
    ...
    def _train_classifier(self, t, trn_loader, val_loader, name='classifier'):
        ...
        _task_classifierexp.to(self.device)
        lr = 5e-3#self.head_classifier_lr
