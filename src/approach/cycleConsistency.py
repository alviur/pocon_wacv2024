...
class Appr(Learning_Appr):
    ...
    def extra_parser(args):
        ...
        parser.add_argument("--kd_method", default="ft", type=str, choices=['ft', 'L2', 'EWC', 'p2_f','EWC_p2','cycle'])
    ...
    def train(self, t, trn_loader, val_loader):
        ...
        if t == 0:
            ...
            torch.distributed.init_process_group(backend='nccl', init_method='tcp://localhost:10027',
...
class BarlowTwins(nn.Module):
    def __init__(self, projectorArc,batch_size,lambd,change_lr_scheduler,maxEpochs,diff_lr,kd_method,lambdap2):
        ...
        if self.kd == 'cycle':
            self.p2 = self._prediction_mlp(512, 256)
            self.b2 = self._prediction_mlp(512, 256)
    ...
    def forward(self, x1, x2):
        ...
        if self.t > 0 and self.kd == 'cycle':
            ...
            # lossKD1 = 20*(-(self.criterion(p2_1, f1Old.detach()).mean()
            #                             + self.criterion(p2_2, f2Old.detach()).mean()) * 0.5)
            #
            # lossKD2 = 10*(-(self.criterion(b2_1, f1).mean() + self.criterion(b2_2, f2).mean()) * 0.5)

            lossKD1 = (-(self.criterion(p2_1, f1Old.detach()).mean()
                         + self.criterion(p2_2, f2Old.detach()).mean()) * 0.5)

            p2_1_1 = self.p2(f1)
            p2_2_1 = self.p2(f2)

            b2_1_1 = self.b2(p2_1_1)
            b2_2_1 = self.b2(p2_2_1)

            lossKD2 = (-(self.criterion(b2_1_1, f2).mean() + self.criterion(b2_2_1, f1).mean()) * 0.5)


            # # if self.wandblog:
            # wandb.log({"KD loss": (self.lambdap2 * (lossKD1+lossKD2)).item()})

            loss +=  self.lambdap2 * (lossKD1+lossKD2)
            ...
            lossKD = self.lambdap2 * (torch.dist(f1Old.detach(), f1)+torch.dist(f2Old.detach(), f2))
            print(self.lambdap2,lossKD)
            # print(self.lambdap2,lossKD,loss)
