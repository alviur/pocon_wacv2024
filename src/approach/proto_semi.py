...
from .utilsProj import load_backboneProj
import sys
...
class Appr(Learning_Appr):
    ...
    def train(self, t, trn_loader, val_loader, output_path):
        ...
        if t == 0:
            ...
            self.modelFB.CurrentexpertBackbone = deepcopy(self.modelFB.expertBackbone)
            ...
            if self.loadTask1:
                ...
                self.modelFB = load_backboneProj(self.modelFB, modelDict['simsiam'])
            ...
            self.optimizer, self.scheduler = self.modelFB.configure_optimizers(1,0)
    ...
    # Compute protoypes using current data and current backbone
    # create evalPrototypes function
    def findPrototypes(self, t, trn_loader, val_loader):
        ...
        # Subsample for semi-supervised
        subset = 0.008
        percentage = int(subset*trainY.shape[0])


        protoDict = torch.from_numpy(np.zeros((valY.max()+1,512))).cuda()
        contProtodict = torch.from_numpy(np.zeros((valY.max()+1,1))).cuda()


        # be sure there is at least one sample per class!!
        notBalanced = True
        while notBalanced:
            subsampleIndex = torch.randint(0, trainY.shape[0], (percentage,))
            contClasses = torch.from_numpy(np.zeros((valY.max()+1,1)))
            for i in range(subsampleIndex.shape[0]):
                # if contClasses[trainY[subsampleIndex[i]]] < 1:
                if contClasses[trainY[subsampleIndex[i]]] >= 0:
                    # contClasses[trainY[subsampleIndex[i]]] = 1
                    contClasses[trainY[subsampleIndex[i]]] += 1
            #

            # if contClasses.sum() == contClasses.shape[0]:
            for j in range(contClasses.shape[0]):

                if contClasses[j] >= int(percentage/(valY.max()+1)):
                    # import pdb;
                    # pdb.set_trace()
                    contClasses[j] = 1
                else:
            ...
            if contClasses.sum() == contClasses.shape[0]:
                ...
                print("Resampling...",contClasses)

        for i in range(subsampleIndex.shape[0]):
            protoDict[trainY[subsampleIndex[i]]] = protoDict[trainY[subsampleIndex[i]]] + trainX[subsampleIndex[i],:]
            contProtodict[trainY[subsampleIndex[i]]] += 1



        self.protoDict = torch.div(protoDict, contProtodict)

        accu = self.evalPrototypes(t,valX, valY)

        return accu,self.protoDict

    def mapPrototypes(self,oldPrototypes,t):

        for i in range(oldPrototypes.shape[0]):

            seedProto = Variable(torch.randn(512,512).cuda(), requires_grad=True)
            target = oldPrototypes[i,:]
            lr = 0.1
            proto_optimizer = torch.optim.Adam([seedProto], lr=lr)
            # proto_optimizer = torch.optim.SGD([seedProto], lr=lr, momentum=0.9)
            # proto_optimizer = torch.optim.AdamW([seedProto], lr=lr)


            lossProto = nn.MSELoss()

            optimepochs = 5000
            for j in range(optimepochs):

                prevPos = self.modelFB.projectorRet(seedProto)
                loss = lossProto(target.type(torch.DoubleTensor) ,prevPos[0,:].type(torch.DoubleTensor) )
                # update finetune weights
                loss.backward()
                proto_optimizer.step()
                proto_optimizer.zero_grad()

            oldPrototypes[i,:] = prevPos[0,:].detach()

        return oldPrototypes

    def evalPrototypes(self,t,valX, valY):

        accu = 0

        for i in range(valY.shape[0]):

            dist = torch.cdist(torch.unsqueeze(self.protoDict,0).double(), torch.unsqueeze(torch.unsqueeze(valX[i,:],0),0).double(), p=2)
            pred = torch.argmin(dist)

            if pred == valY[i]:
                accu +=1

        accu = accu/valY.shape[0]
        # import pdb;
        # pdb.set_trace()

        return accu

    def evaldriftedPrototypes(self,t,prototypes,trn_loader,val_loader):

        for task in range(t+1):
            _, _, valX, valY, _, _ = self.get_embeddings(task, trn_loader[task], val_loader[task])

            valY = valY + self.model.task_offset[task]
            ...
            if task == 0:
                allValX = valX
                allValY = valY
                totalSamples = valY.shape[0]
                allProto = prototypes[task]
            else:
                allValX = torch.cat((allValX, valX), 0)
                allValY = torch.cat((allValY, valY), 0)
                totalSamples = totalSamples + valY.shape[0]

                allProto =  torch.cat((allProto, prototypes[task]), 0)

        accu = 0
        # import pdb;
        # pdb.set_trace()

        for i in range(allValY.shape[0]):

            dist = torch.cdist(torch.unsqueeze(allProto,0).double(), torch.unsqueeze(torch.unsqueeze(allValX[i,:],0),0).double(), p=2)
            pred = torch.argmin(dist)
        ...
        # accu = accu/valY.shape[0]
        print("Drifted Accu task ",t,accu)


        return accu/totalSamples
