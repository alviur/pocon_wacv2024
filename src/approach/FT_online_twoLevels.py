...
class Appr(Learning_Appr):
    ...
    def extra_parser(args):
        ...
        parser.add_argument("--expertArch", default="ResNet9", type=str, choices=['ResNet9', 'ResNet18','SqueezeNet'])
    ...
    def train(self, t, trn_loader, val_loader, num_classes, omni_trn_loader, omni_val_loader, omni_tst_loader,output_path):
        ...
        if self.sslModel == "BT":
            self.modelFB = BarlowTwins(self.projectorArc, self.batch_size, self.lambd, self.change_lr_scheduler,
                                       ...
                                       self.norm, self.mainArch,self.omni_trn_loader,self.omni_val_loader,self.val_transforms,self.omni_tst_loader,self.test_every_n_steps,output_path)
        ...
        for i, ((x0, x1), _, _) in enumerate(trn_loader):
            ...
            if flagRet and contRetEpochs<self.ret_nepochs:
                ...
                if contRetEpochs >= self.ret_nepochs:
                    ...
                    for param in self.modelFB.backbone.parameters():
                        param.requires_grad = False
                    self.optimizer, self.scheduler = self.modelFB.configure_optimizers(self.lrExpF, 2)
                    self.modelFB.copyBackbone = self.modelFB.backbone
                    for param in self.modelFB.copyBackbone.parameters():
                        param.requires_grad = False
                    flagTrans = True

            if flagTrans and contTransEpochs < self.trans_nepochs:
                self.modelFB.transfer = True
                contTransEpochs += 1
                if contTransEpochs >= self.trans_nepochs:
                    contTransEpochs = 0
                    flagTrans = False
