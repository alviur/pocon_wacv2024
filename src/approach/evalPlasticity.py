...
class Appr(Learning_Appr):
    ...
    def train(self, t, trn_loader, val_loader, output_path):
        ...
        # path = self.pathModelT1 + '/resnet18_'+str(t)+'.pth'
        # path = self.pathModelT1 + 'model_expert_' + str(t) + '.pth'
        # path = self.pathModelT1 + 'model_main_' + str(t) + '.pth'
        # path = self.pathModelT1 + 'checkpoint_mode1task0.pth'
    ...
    def get_embeddings(self, t, trn_loader, val_loader):
        ...
        # modelT = deepcopy(self.modelFB.expertBackbone).to(self.device)
        modelT = deepcopy(self.modelFB.currentExpert).to(self.device)
        modelTexpert = deepcopy(self.modelFB.currentExpert
                                ).to(self.device)
