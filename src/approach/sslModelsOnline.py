...
class BarlowTwins(nn.Module):
    def __init__(self, projectorArc, batch_size, lambd, change_lr_scheduler, maxEpochs, diff_lr, kd_method, lambdapRet,
                 ...
                 omni_val_loader,val_transforms,omni_tst_loader,test_every_n_steps,output_path,expEmbDim):
        super().__init__()
        ### Barlow Twins params ###
        ...
        self.p2 = utilsProj.prediction_mlp(512, 256, 512, self.linearProj)
        ...
        self._encoderexp_emb_dim = expEmbDim

        sizes = [self._encoderexp_emb_dim] + list(map(int, self.projectorArc.split('_')))
        layers = []
        ...
        # Projectors
        self.p2 = utilsProj.prediction_mlp(512, 256, 512,self.linearProj)
        self.p2exp = utilsProj.prediction_mlp(512, 256, 512, self.linearProj)
        self.bnExp = nn.BatchNorm1d(self.expProjSize, affine=False)
        self.bnRet = nn.BatchNorm1d(self.expProjSize, affine=False)
        self.projectorExp = utilsProj.prediction_mlp(self.expProjSize, 256, self._encoderexp_emb_dim, self.linearProj)
        self.projectorRet = utilsProj.prediction_mlp(self.expProjSize, int(self.expProjSize / 2), 512,self.linearProjRet)
    ...
    def forward(self, x1, x2, batch_idx):
        ...
        if (1):
            ...
            if self.kd == 'L2' and self.retrospection:
                ...
                if self.oldBackbone != None:
                    # print("using old backbone")
            ...
            # if self.kd == 'L2' and self.transfer:
            #
            ...
            # if self.kd == 'L2' and self.transferL2:
            #
            #     f1 = torch.squeeze(self.copyBackbone(x1)).detach()
            #     f2 = torch.squeeze(self.copyBackbone(x2)).detach()
            #
            #     f1Expert = torch.squeeze(self.backboneL2(x1))
            #     f2Expert = torch.squeeze(self.backboneL2(x2))
            #
            #     z1 = self.projectorExpL2(f1Expert)
            #     z1_2 = self.projectorExpL2(f2Expert)
            #
            #     # ======L2
            #     lossTransferL2 = self.lambdaExp * (
            #             torch.dist(torch.nn.functional.normalize(z1), torch.nn.functional.normalize(f1)) +
            #             torch.dist(torch.nn.functional.normalize(z1_2), torch.nn.functional.normalize(f2)))
            #
            #     loss += lossTransferL2
            #
            #     wandb.log({"loss transfer L2": lossTransferL2.item()})
            #
            #     if self.oldBackboneL2 != None:
            #         # print("using old backbone")
            #
            #         f1old = torch.nn.functional.normalize(torch.squeeze(self.oldBackboneL2(x1))).detach()
            #         f2old = torch.nn.functional.normalize(torch.squeeze(self.oldBackboneL2(x2))).detach()
            #
            #
            #         # Map to old task
            #         z1 = self.projectorRetL2(f1Expert)
            #         z1_2 = self.projectorRetL2(f2Expert)
            #
            #         # ======L2 + ssL2 norm
            #         lossRetroL2 = self.lambdapRet * (
            #                     torch.dist(torch.nn.functional.normalize(z1), torch.nn.functional.normalize(f1old)) +
            #                     torch.dist(torch.nn.functional.normalize(z1_2), torch.nn.functional.normalize(f2old)))
            #
            #         loss += lossRetroL2
            #         wandb.log({"loss Retrospection L2": lossRetroL2.item()})
    ...
    def _train_Expert(self, t, trn_loader, val_loader, name='classifier'):
        ...
        # Linear
        _task_classifier = SSLEvaluator(self._encoderexp_emb_dim, _num_classes, 0, 0.0)
        _task_classifierexp = SSLEvaluator(self._encoderexp_emb_dim, _num_classes, 0, 0.0)
        ...
        for e in range(self.classifier_nepochs):
            ...
            while index + trn_loader.batch_size <= self.trainX[str(t)].shape[0]:
                ...
                _x = _x.detach()
                # forward pass
                mlp_preds = _task_classifierexp(_x.cuda())
    ...
    def get_embeddings(self, t, trn_loader, val_loader):
        ...
        trainXexp = torch.zeros((batchFloorT, self._encoderexp_emb_dim), dtype=torch.float).cuda()
        valXexp = torch.zeros((batchFloorV, self._encoderexp_emb_dim), dtype=torch.float).cuda()
    ...
    def configure_Dualoptimizers(self, lrExp, mode):
        ...
        optimExp = torch.optim.SGD(paramExp, lr=self.base_lr, momentum=0.9, weight_decay=5e-4)
        optimMain = torch.optim.SGD(paramExp, lr=self.base_lr, momentum=0.9, weight_decay=5e-4)

        return optimExp, optimMain
