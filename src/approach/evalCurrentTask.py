...
class Appr(Learning_Appr):
    ...
    def extra_parser(args):
        ...
        parser.add_argument("--getExpert", action="store_true")
    ...
    def train(self, t, trn_loader, val_loader, output_path):
        ...
        if t == 0:
            ...
            print("NUMBER OF PARAMS EXPERT BACKBONE: ", sum(p.numel() for p in self.modelFB.expertBackbone.parameters() if p.requires_grad))
        ...
        path = get_ft_file(self.pathModelT1, t)
        ...
        self.modelFB.backbone = deepcopy(load_backbone_cifar_cassle(self.modelFB.backbone, modelDict['state_dict']))
        ...
        with override_dataset_transform(orig_val_loader.dataset, self.test_transforms) as _ds_val:  # no data aug
            ...
            with torch.no_grad():
                ...
                if self.getExpert:
                    ...
                    modelTProj = deepcopy(self.modelFB.currentExpert).to(self.device)
                else:
                    modelT = deepcopy(self.modelFB.backbone).to(self.device)
                    modelTexp = deepcopy(self.modelFB.backbone).to(self.device)
                    modelTProj = deepcopy(self.modelFB.backbone).to(self.device)
    ...
    def get_embeddings(self, t, trn_loader, val_loader):
        ...
        if self.getExpert:
            ...
            modelT = deepcopy(self.modelFB.backbone).to(self.device)
            modelTexpert = deepcopy(self.modelFB.backbone).to(self.device)
        ...
        with override_dataset_transform(trn_loader.dataset, self.val_transforms) as _ds_train, \
            ...
            for img_1, y in _train_loader:
                ...
                y = torch.LongTensor((y-self.offset*t).long().cpu()).to(self.device)
...
def load_backbone(model, stateDictSaved):
    ...
    for name, param in model.state_dict().items():
        ...
        # if (name[0:8]=='import pdb; pdb.set_trace()') or (name[0:14]=='expertBackbone') or (name[0:14]=='currentExpert'):
        print("---loading", add+name)
        model.state_dict()[name].copy_(stateDictSaved[add+name])
...
def get_ft_file(pathFT, task):

    prev = "/barlow-cifar100-cassle-split:50-task"
    post = "-ep=499"
    # DomainNet
    # prev = "/barlow-domainnet-task"# Domain
    # post = "-ep=199"
    files = os.listdir(pathFT)


    for file in files:
        if (os.path.isdir(pathFT + file)):
            models = os.listdir(pathFT + file)
            for model in models:
                # print(model)
                if model[-5:] == ".ckpt":
                    # print(model,model[model.find("task")+4:])
                    mixedPath = pathFT + file + prev + str(task) + model[model.find(post):]
                    # print(model,prev +str(task) +model[28:])
                    if os.path.isfile(mixedPath):
                        return mixedPath

def load_backbone_cifar_cassle(model, stateDictSaved):
    # own_state = model.state_dict()
    add = "encoder."
    for name, param in model.state_dict().items():
        # # debug python
