...
class Appr(Learning_Appr):
    ...
    def __init__(
        ...
        if self.wandblog:
            wandb.init(project="Barlow Twins FB Multi-GPU",tags=["Barlow_twins", "Tiny-Imagenet", "Multi-GPU"])
            # wandb.log({"Parameters": parameters})
    ...
    def extra_parser(args):
        ...
        parser.add_argument("--kd_method", default="ft", type=str, choices=['ft', 'L2', 'L2rel', 'L2relCos', 'p2', 'EWC', 'p2_f','EWC_p2','lwf_v3'])
    ...
    def get_data_loaders(self, trn_loader, val_loader, t):  # -> Replace _prepare_transformations
        ...
        collate_fn = lightly.data.SimCLRCollateFunction(input_size=224,normalize=imagenet_normalize)
        ...
        #normalization = transforms.Normalize((0.5071, 0.4866, 0.4409), (0.2009, 0.1984, 0.2023)) # CIFAR
    ...
    def train(self, t, trn_loader, val_loader):
        ...
        if t == 0:
            ...
            torch.distributed.init_process_group(backend='nccl', init_method='tcp://localhost:10009',
                                                 world_size=1, rank=0)

            # Create data loaders for data in t
            self.get_data_loaders(trn_loader, val_loader, t)

            # # Create Barlow twins architecture
            ...
            # See gradients of backbone
            wandb.watch(self.modelFB.oldBackbone, log_freq=1)
            ...
            if self.loadTask1:
                ...
                #path = '/home/agomezvi/simSiamLB2/tinyModels512/'
                #path = '/home/agomezvi/Desktop/simSiamCorrections/cvc-class-il/cifar100_noTrans_simsiam_simsiam_p2-10072021_150917_368433/'
                #modelDict = torch.load(path + 'model-task_0-time_1634729034.ckpt', map_location='cuda:0')
            ...
            self.modelFB = nn.DataParallel(self.modelFB)
            self.modelFB.cuda()
        else:
            # Create data loaders for data in t
            self.get_data_loaders(trn_loader, val_loader, t)
            # self.modelFB.module.t = t
            # self.optimizer, self.scheduler = self.modelFB.module.configure_optimizers()
            self.modelFB.t = t
            self.optimizer, self.scheduler = self.modelFB.configure_optimizers()

            if t > 0:
                print("Checking backbone============================")
                for p1, p2 in zip(self.oldBackbone.parameters(), self.modelFB.backbone.parameters()):
            ...
            self.modelFB = nn.DataParallel(self.modelFB)
            # self.modelFBNew = BarlowTwins(self.projectorArc, self.batch_size, self.lambd, self.change_lr_scheduler,
            #                            self.nepochs, self.diff_lr, self.kd_method, self.lambdap2)
            #
            # self.modelFBNew.t = t
            # self.optimizer, self.scheduler = self.modelFBNew.configure_optimizers()
            # #self.modelFBNew.oldBackbone = deepcopy(self.modelFBNew.backbone)
            # load_my_state_dict(self.modelFBNew,self.modelFB.module.state_dict())
            # del self.modelFB
            #
            # self.modelFB = deepcopy(self.modelFBNew)
            # del self.modelFBNew
            #
        ...
        if t == 0 and self.task1_nepochs > 0:
            nepochs = self.task1_nepochs
        ...
        if(t>=0):
            for epochs in range(nepochs):
                # Run 1 epoch
                if self.loadTask1 and t<1:
                    continue
                else:
                    print("NCCL-------------------", torch.distributed.is_nccl_available())
                    print(torch.__version__)
                    self.train_loop(t, trn_loader, val_loader, epochs)

        # Return model do single GPU
        self.modelFB = peel_model(self.modelFB)



        # Save old models
        self.modelFB.oldBackbone = deepcopy(self.modelFB.backbone)
        self.oldBackbone = deepcopy(self.modelFB.backbone)
        for param in self.modelFB.oldBackbone.parameters():
            param.requires_grad = False
        #self.oldBackbone = deepcopy(self.modelFB.module.backbone)

        # Fisher and importance matrices for EWC and MAS
        if self.kd_method == "EWC":
            # Extract values for EWC
            with override_dataset_transform(trn_loader.dataset, self.val_transformsEWC) as _ds_train:
                batch_size = 200
                ...
                self.modelFB.module.post_train_process(t, trainloader)
        ...
        if(t>=0):
            ...
            save_checkpoint({
                'epoch': str(epochs),
                'arch': 'ResNet18',
                'state_dict': self.modelFB.state_dict(),
                'optimizer': self.optimizer.state_dict(),
            }, is_best=False, filename=path + 'task_' + str(
                t) + '.pth.tar')
    ...
    def train_loop(self, t, trn_loader, val_loader, epoch):
        ...
        for i, ((x0, x1), _, _) in enumerate(self.dataloader_train_ssl):
            ...
            if t>0 and self.kd_method == "p2":
                p1, z2, p2, z1, p2_1, z1Old, p2_2, z2Old = self.modelFB(x1=x0.to(self.device), x2=x1.to(self.device))
                loss = -(self.criterion(p1, z2.detach()).mean() + self.criterion(p2, z1.detach()).mean()) * 0.5
                wandb.log({"Contrast loss": loss.item()})
                lossKD = self.lambdap2 * (-(self.criterion(p2_1, z1Old).mean() + self.criterion(p2_2, z2Old).mean()) * 0.5)
                loss += lossKD
                wandb.log({"KD loss": lossKD.item()})
            elif t>0 and self.kd_method == "L2":
                p1, z2,p2, z1,f1Old,f2Old,f1,f2 = self.modelFB(x1=x0.to(self.device), x2=x1.to(self.device))
                loss = -(self.criterion(p1, z2.detach()).mean() + self.criterion(p2, z1.detach()).mean()) * 0.5
                wandb.log({"Contrast loss": loss.item()})
                lossKD = self.lamb[self.t - 1, 0] * (torch.dist(f1Old, f1)+torch.dist(f2Old, f2))
                loss += lossKD
                wandb.log({"KD loss": lossKD.item()})
            elif t>0 and self.kd_method == "p2_f":

                z1, z2, p2_1, p2_2, f1Old, f2Old  = self.modelFB(x1=x0.to(self.device), x2=x1.to(self.device))

                lossKD = self.lambdap2 * (-(self.criterion(p2_1, f1Old.detach()).mean()
                                            + self.criterion(p2_2, f2Old.detach()).mean()) * 0.5)

                # empirical cross-correlation matrix
                c = self.modelFB.module.bn(z1).T @ self.modelFB.module.bn(z2)
                # sum the cross-correlation matrix between all gpus
                c.div_(z1.shape[0])
                torch.distributed.all_reduce(c)
                on_diag = torch.diagonal(c).add_(-1).pow_(2).sum()
                off_diag = off_diagonal(c).pow_(2).sum()
                # Compute loss in a single GPU!
                lossC = (0.025 * (on_diag + 0.0051 * off_diag))

                wandb.log({"Contrast loss": lossC.item(),"KD loss": lossKD.item(),"Tasks": 0})
                loss = lossC + lossKD

            elif t>0 and self.kd_method == "EWC":
                p1, z2, p2, z1 , fisher,older_params,backbone= self.modelFB(x1=x0.to(self.device), x2=x1.to(self.device))
                loss = -(self.criterion(p1, z2.detach()).mean() + self.criterion(p2, z1.detach()).mean()) * 0.5
                wandb.log({"Contrast loss": loss.item()})
                # lossKD = self.modelFB.criterionEWC(self.t)
                loss = 0
                loss_reg = 0
                ...
                for n, p in backbone.named_parameters():
                    if n in fisher.keys():
                        loss_reg += torch.sum(
                            fisher[n].cuda() * (p - older_params[n].cuda()).pow(2)) / 2
                lossKD = 10000 * loss_reg
                loss += lossKD
                wandb.log({"KD loss": lossKD.item()})
            else:
                # loss =  self.modelFB(x1=x0.to(self.device), x2=x1.to(self.device))
                z1, z2 = self.modelFB(x1=x0.to(self.device), x2=x1.to(self.device))
                ...
                # Compute loss in a single GPU!
                loss = (0.1 * (on_diag + 0.0051 * off_diag))
                # print(i,len(self.dataloader_train_ssl))

                wandb.log({"Contrast loss": loss.item(),"Tasks": 0})
...
class BarlowTwins(nn.Module):
    def __init__(self, projectorArc,batch_size,lambd,change_lr_scheduler,maxEpochs,diff_lr,kd_method,lambdap2):
        ...
        self.oldBackbone = deepcopy(self.backbone)

        for param in self.oldBackbone.parameters():
            param.requires_grad = False
        ...
        # P2
        self.p2 = None
        if self.kd == 'p2' or self.kd == 'p2_f' :
            self.p2 = self._prediction_mlp(512, 256)
        self.lambdap2 = lambdap2
        self.oldProjector = None
    ...
    def forward(self, x1, x2):
        ...
        # print("This should be =0", torch.sum(torch.abs(f1Old - f1)))

        # if self.t>0:
        #     print("Checking backbone============================")
        #     for p1, p2 in zip(self.oldBackbone.parameters(), self.backbone.module.parameters()):
        #         if p1.data.ne(p2.data).sum() > 0:
        #             print("models are different!!")
        #     print("Equal!!")
        if self.t > 0 and self.kd == 'L2':
            ...
            return on_diag, off_diag,self.lambd,f1Old,f2Old,f1,f2
        #
        elif self.t > 0 and self.kd == 'EWC':            # loss = 0
            # loss_reg = 0
            # # Eq. 3: elastic weight consolidation quadratic penalty
            # for n, p in self.modelFB.backbone.named_parameters():
            #     if n in self.modelFB.fisher.keys():
            #         loss_reg += torch.sum(
            #             self.modelFB.fisher[n].cuda() * (p - self.modelFB.older_params[n].cuda()).pow(
            #                 2)) / 2
            # lossKD = 10000 * loss_reg
            ...
            return on_diag, off_diag,self.lambd, self.fisher,self.older_params,self.backbone.cuda()

        elif  self.t > 0 and self.kd == 'p2_f':
            ...
            return z1,z2, p2_1, p2_2, f1Old.detach(), f2Old.detach()
            ...
            # return (0.025 * (on_diag + 0.0051 * off_diag))
            return z1,z2
    ...
    def configure_optimizers(self):
        ...
        if self.t < 1:
            ...
            optim = torch.optim.SGD(params, lr=lr, momentum=0.9, weight_decay=5e-4)
            if self.maxEpochs < 600:
            ...
            lr = self.base_lr
            params = [
                {'params': self.backbone.parameters(), 'lr': lr*0.01 if self.diff_lr else 0.8*0.5*lr},
            ...
            #optim = torch.optim.SGD(params, lr=lr * 0.8, momentum=0.9, weight_decay=0.001)
    ...
    def criterionEWC(self, t):
        ...
        if t > 0:
            ...
            loss += self.lamb[self.t - 1, 0] * loss_reg
    ...
    def compute_fisher_matrix_diag(self, trn_loader, t):
        ...
        for ((inputs, inputsAug, _), targets) in itertools.islice(trn_loader, n_samples_batches):
            ...
            if self.sampling_type == 'ce':
                ...
                (out0, r0), (out1, r1) = self.resnet_simsiam(inputs.cuda(), inputsAug.cuda(),
                                                             return_features=True)
                loss = self.criterion(out0, out1)

                if t > 0:
                    (out0old, r1), (out1old, r2) = self.oldModelFull(inputs.cuda(), inputsAug.cuda(),
                                                                     return_features=True)
                    zold0 = out0old[1]
                    zold1 = out1old[1]
                    pold0 = self.p2(out0[1])
                    pold1 = self.p2(out1[1])

                    lossKD_p2 = 1 * (
                            self.cosine_similarity(pold0, zold0) / 2 + self.cosine_similarity(pold1, zold1) / 2
                    )
