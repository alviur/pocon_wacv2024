...
# Split dataset
def prepare(rank, world_size, sslDataloader, batch_size=32, pin_memory=False, num_workers=0):

    dataset = sslDataloader.dataset
...
def trainDPP(rank, world_size, t, trn_loader, val_loader,output_path, model):

    print("===================", len(trn_loader.dataset),"Pytorch", torch.__version__)

    # modelSSL = deepcopy(model).cuda(rank)

    modelSSL =  BarlowTwins('2048_2048_2048', 512, 1, False,
                30, True, 'L2', 1, 1,
                0.01, 512, 1, 3,3,
                True, "ResNet18", 1)


    modelSSL.expertBackbone = models.resnet18(pretrained=False, zero_init_residual=True)
    modelSSL.expertBackbone.fc = nn.Identity()
    modelSSL.expertBackbone.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=2, bias=False)
    modelSSL.expertBackbone.maxpool = nn.Identity()


    modelSSL.projectorExp = utilsProj.prediction_mlp(512, 256, 3)
    modelSSL.projectorRet = utilsProj.prediction_mlp(512, 256, 3)

    modelSSL.retrospection = False
    modelSSL.tranfer = False
    ...
    per_Device_bach = int(512/world_size)
    # dataloader = prepare(rank, world_size, trn_loader, batch_size=per_Device_bach, pin_memory=False, num_workers=0)
    ...
    sampler = DistributedSampler(dataset, num_replicas=world_size, rank=rank, shuffle=True, drop_last=False)
    dataloader = DataLoader(dataset, batch_size=per_Device_bach, pin_memory=False, num_workers=0,
                            drop_last=False, sampler=sampler)

    modelSSL = nn.SyncBatchNorm.convert_sync_batchnorm(modelSSL)

    params = [
        {'params': modelSSL.projector.parameters()},
        {'params': modelSSL.backbone.parameters()},
        {'params': modelSSL.expertBackbone.parameters()},

    ]

    # optimizer, scheduler = modelSSL.configure_optimizers(1, 0)

    modelSSL = DDP(modelSSL, device_ids=[rank], output_device=rank, find_unused_parameters=True)

    optimizer = torch.optim.SGD(params, lr=0.01, momentum=0.9, weight_decay=5e-4)

    stats_file = open(output_path +  'stats.txt', 'a', buffering=1)
    # print(' '.join(sys.argv))
    ...
    for epochs in range(nepochs):
        ...
        for step, ((y1, y2), _) in enumerate(dataloader):

            y1 = y1.cuda(rank)
            y2 = y2.cuda(rank)

            # if rank == 0:
            #     print(y1.max(),y1.min(),y2.max(),y2.min(),torch.abs(y1-y2).sum())
            # compute output and loss
            loss = modelSSL(x1=y1, x2=y2)
            ...
            if step % 10 == 0:
                if rank == 0:
                    stats = dict(epoch=epochs, step=step,loss=loss.item(),
    ...
    if rank == 0:
        modelSSL = peel_model(modelSSL)
        models_to_save = {
            'simsiam': modelSSL.state_dict()
        }
        torch.save(models_to_save, output_path + '/model-task_0.ckpt')
        # torch.save(modelSSL.module.expertBackbone.state_dict(), output_path + '/DDP_backbone.pth')
        print(output_path)
...
class Appr(Learning_Appr):
    ...
    def train(self,t, trn_loader, val_loader,output_path):
        ...
        mp.spawn(trainDPP, args=(world_size, t, trn_loader, val_loader,output_path, self.modelFB), nprocs=world_size )
