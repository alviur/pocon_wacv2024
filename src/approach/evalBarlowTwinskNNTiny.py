...
class Appr(Learning_Appr):
    ...
    def train(self, t, trn_loader, val_loader):
        ...
        #path = '/home/agomezvi/simSiamLB2/modelsProj512/4_tasks/tiny/EWC/'  # EWC
        #path = '/home/agomezvi/simSiamLB2/modelsProj512/4_tasks/tiny/P2/'  # P2
        path = "/home/agomezvi/simSiamLB2/4_tasks/tiny/pfr_l50/" # PFR
        #path = "/home/agomezvi/simSiamLB2/4_tasks/tiny/FT/"  # FT
        ...
        with override_dataset_transform(orig_val_loader.dataset, self.test_transforms) as _ds_val:  # no data aug
            ...
            with torch.no_grad():
                ...
                for img_1, targets in val_loader:
                    ...
                    valX[contBatch:contBatch + val_loader.batch_size, :] = r1
                    valY[contBatch:contBatch + val_loader.batch_size] = targets
                    ...
                    contBatch += val_loader.batch_size

            io.savemat('valX_' + str(t) + '.mat', {"valX_" + str(t): valX.cpu().numpy()})
            io.savemat('valY_' + str(t) + '.mat', {"valY_" + str(t): valY.cpu().numpy()})

            np.save("valX_" + str(t), valX.cpu().numpy())
            np.save("valY_" + str(t), valY.cpu().numpy())
    ...
    def _train_classifier(self, t, trn_loader, val_loader, name='classifier'):
        ...
        io.savemat('trainX_'+str(t)+'.mat', {"trainX_"+str(t): trainX.cpu().numpy()})
        io.savemat('trainY_'+str(t)+'.mat', {"trainY_"+str(t): trainY.cpu().numpy()})

        np.save("trainX_"+str(t), trainX.cpu().numpy())
        np.save("trainY_"+str(t), trainY.cpu().numpy())
    ...
    def train_downstream_classifier(self, t, trn_loader, val_loader, name='downstream-task-classifier'):
        ...
        knnAccu = self.validateKNN(trn_loader, val_loader, k=100, t=0.1, task=t)
        self.knn.append(knnAccu)
        print("kNN", self.knn)

        return self._train_classifier(t, trn_loader, val_loader, name)

    def validateKNN(self, train_loader, val_loader, k=50, t=0.1, task = 1):
        modelT = deepcopy(self.modelFB.backbone).to(self.device)
        for param in modelT.parameters():
            param.requires_grad = False
        modelT.eval()
