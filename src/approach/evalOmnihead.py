...
class Appr(Learning_Appr):
    ...
    def extra_parser(args):
        ...
        parser.add_argument("--expertArch", default="ResNet9", type=str, choices=['ResNet9', 'ResNet18','mobilenet',
                                                                                  'shufflenet'])
    ...
    def eval(self, t, orig_val_loader, heads_to_evaluate=None):
        with override_dataset_transform(orig_val_loader.dataset, self.test_transforms) as _ds_val:  # no data aug
            ...
            with torch.no_grad():
                ...
                # modelT = deepcopy(self.modelFB.curr).to(self.device)
                modelT = deepcopy(self.modelFB.backbone).to(self.device)
...
def get_ft_file(pathFT, task):
    ...
    # prev = "/barlow-cifar100-task"

    prev = "/barlow100-cifar100-decorrelative-task"
    post = "-ep=499"
    # DomainNet
