...
from .sslModels import BarlowTwins,vicReg,SqueezeNet
import sys
...
class Appr(Learning_Appr):
    ...
    def __init__(
        ...
        self.task1_Retnepochs = task1_Retnepochs
        self.beginAtTask = beginAtTask
        ...
        if self.wandblog:
            wandb.init(project="Cache Networks L2")
            wandb.log({'Parameters': wandb.Table(dataframe=parameters)})

        self.lamb = np.ones((20, 1)) * lamb
        # Decreasing lambda
        for i in range(1, 21):
            self.lamb[i - 1] = np.sqrt(20 / (i * 20)) * 0.01
    ...
    def extra_parser(args):
        ...
        parser.add_argument("--dataset2", default="cifar100", type=str, choices=['cifar100','cifar10', 'tiny', 'imagenet100'])
        ...
        parser.add_argument("--expertArch", default="ResNet9", type=str, choices=['ResNet9', 'ResNet18','SqueezeNet','ResNet18Com'])
        parser.add_argument("--mainArch", default="ResNet18", type=str, choices=['ResNet9', 'ResNet18','SqueezeNet','ResNet18Com'])
        parser.add_argument("--sslModel", default="BT", type=str, choices=['BT', 'mocov2', 'vicreg'])
        ...
        parser.add_argument("--linearProjRet", type=int, default=0)
        ...
        parser.add_argument("--task1_Retnepochs", type=int, default=500)
        parser.add_argument("--beginAtTask", type=int, default=0)
    ...
    def get_data_loaders(self, trn_loader, val_loader, t):  # -> Replace _prepare_transformations
        ...
        elif self.dataset2 == 'cifar100' or self.dataset2 == 'cifar10':
            input_height = 32
        ...
        # Imbalance training here
        # unbalanceRatio = [float(x) for x in  self.unbalanceRatio]
        # unbalanceRatioF = [1.0, 0.5, 1.0, 0.5, 1.0, 0.5, 1.0, 0.5, 1.0, 0.5]
        # unbalanceRatioF = [0.5, 1.0, 0.3, 0.5, 1.0, 0.2, 0.7, 0.8, 1.0, 0.5]
        # subset = trn_loader.dataset.splitInbalance(unbalanceRatioF)
    ...
    def train(self, t, trn_loader, val_loader,output_path):
        ...
        print("===================", len(trn_loader.dataset), self.lr, " expertArch:",self.expertArch, " mainArch:",self.mainArch, self.dataset2, ' Adapt sche: ',
              self.adaptSche, " Linear projector:",self.linearProj,"Pytorch", torch.__version__,"PL:",pl.__version__, "lightly",lightly.__version__)
        ...
        if t == 0:
            ...
            if self.sslModel == "BT":
                self.modelFB = BarlowTwins(self.projectorArc, self.batch_size, self.lambd, self.change_lr_scheduler,
                                           ...
                                           self.lr, self.expProjSize, self.adaptSche, self.linearProj, self.linearProjRet,
                                           self.norm, self.mainArch,self.lamb)
            elif self.sslModel == 'vicreg':
                self.modelFB = vicReg(self.projectorArc, self.batch_size, self.lambd, self.change_lr_scheduler,
                                           self.nepochs, self.diff_lr, self.kd_method, self.lambdapRet, self.lambdaExp,
            ...
            elif self.expertArch == 'ResNet18Com':
                self.modelFB.expertBackbone = models.resnet18(pretrained=False, zero_init_residual=True)
                self.modelFB.expertBackbone.fc = nn.Identity()

            elif self.expertArch == 'ResNet9':
                self.modelFB.expertBackbone = ResNet9()

            elif self.expertArch == 'SqueezeNet':
                self.modelFB.expertBackbone = SqueezeNet()

            self.modelFB.projectorExp = utilsProj.prediction_mlp(512, 256, self.linearProj)
            self.modelFB.projectorRet = utilsProj.prediction_mlp(512, 256, self.linearProjRet)
            ...
            if self.loadTask1:
                ...
                # self.modelFB.backbone = deepcopy(self.modelFB.expertBackbone)
            ...
            if t < self.beginAtTask:
                temp = deepcopy(self.modelFB.backbone)
        ...
        ## Training
        print("--------Training phase-------> train for ",nepochs, self.nepochs, " epochs",t)

        self.modelFB.expertBackbone.apply(set_bn_train)
        for param in self.modelFB.expertBackbone.parameters():
            param.requires_grad = True
        for param in self.modelFB.backbone.parameters():
            param.requires_grad = False

        self.optimizer, self.scheduler = self.modelFB.configure_optimizers(self.lrExpF,0)
        self.modelFB.retrospection = False
        self.modelFB.tranfer = False
        ...
        for epochs in range(nepochs):
            # Run 1 epoch
            if ((self.loadTask1 and t==0) or t < self.beginAtTask) :
                if self.multiGPU:
                    self.modelFB = peel_model(self.modelFB)
                self.modelFB.expertBackbone = deepcopy(self.modelFB.currentExpert)
                print("Loading Expert from memory")
                continue
            else:
        ...
        self.modelFB.currentExpert = deepcopy(self.modelFB.expertBackbone)
        self.modelFB.copyExpert = deepcopy(self.modelFB.expertBackbone)
        ...
        if t >= self.beginAtTask:
            ...
            self.modelFB.copyExpert.apply(set_bn_eval)
            self.modelFB.backbone.apply(set_bn_train)
            self.modelFB.projectorExp = utilsProj.prediction_mlp(512, 256, self.linearProj).to(self.device)

            for param in self.modelFB.copyExpert.parameters():
                param.requires_grad = False
            ...
            if t>0:
                self.modelFB.oldBackbone.apply(set_bn_eval)
                for param in self.modelFB.oldBackbone.parameters():
                    param.requires_grad = False

            # call again optimizers
            self.optimizer, self.scheduler = self.modelFB.configure_optimizers(1,1)
            self.modelFB.retrospection = True
            self.modelFB.transfer = False


            if t==0:
                adaRet_nepochs = self.task1_Retnepochs
            else:
                adaRet_nepochs = self.ret_nepochs
            ...
            for epochs in range(adaRet_nepochs):
                self.train_loop(t, trn_loader, val_loader, epochs)
        ...
        if t >= self.beginAtTask:
            self.modelFB.retrospection = False
            self.modelFB.transfer = True

            self.modelFB.copyBackbone = deepcopy(self.modelFB.backbone)
            self.modelFB.copyBackbone.to(self.device)
            self.modelFB.copyBackbone2 = deepcopy(self.modelFB.backbone)
            self.modelFB.expertBackbone.apply(set_bn_train)
            self.modelFB.copyBackbone.apply(set_bn_eval)

            for param in self.modelFB.expertBackbone.parameters():
                param.requires_grad = True

            for param in self.modelFB.copyBackbone.parameters():
                param.requires_grad = False

            self.optimizer, self.scheduler = self.modelFB.configure_optimizers(1,2)
            ...
            for epochs in range(self.trans_nepochs):
                self.train_loop(t, trn_loader, val_loader, epochs)
            ...
            self.modelFB.backbone = self.modelFB.copyBackbone2
    ...
    def train_loop(self, t, trn_loader, val_loader, epoch):
        ...
        for i, ((x0, x1), _, _) in enumerate(data4Train):
            ...
            losses.update(loss.sum().item(), x0.size(0))
    ...
    def eval(self, t, orig_val_loader, heads_to_evaluate=None):
        with override_dataset_transform(orig_val_loader.dataset, self.test_transforms) as _ds_val:  # no data aug
            ...
            with torch.no_grad():
                ...
                modelTexp = deepcopy(self.modelFB.currentExpert).to(self.device)
                ...
                for param in modelT.parameters():
                    param.requires_grad = False
                for param in modelTexp.parameters():
                    param.requires_grad = False
                ...
                for img_1, targets in val_loader:
                    ...
                    r1exp = torch.nn.functional.normalize(modelTexp(img_1.to(self.device)).flatten(start_dim=1))
                    z1 = modelTProj(r1)
    ...
    def get_embeddings(self, t, trn_loader, val_loader):
        ...
        modelT = deepcopy(self.modelFB.backbone).to(self.device)
        # modelT = deepcopy(self.modelFB.expertBackbone).to(self.device)
        modelTexpert = deepcopy(self.modelFB.currentExpert).to(self.device)
        for param in modelT.parameters():
            param.requires_grad = False
        ...
        return torch.nn.functional.normalize(trainX), trainY, torch.nn.functional.normalize(valX), valY,\
            torch.nn.functional.normalize(trainXexp), torch.nn.functional.normalize(valXexp)
