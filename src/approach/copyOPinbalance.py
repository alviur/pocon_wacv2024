...
class Appr(Learning_Appr):
    ...
    def get_data_loaders(self, trn_loader, val_loader, t):  # -> Replace _prepare_transformations
        ...
        for i in range(len(trainD)):
            if trainD[i][1] not in classesNow.keys():
                classesNow[str(trainD[i][1])] = 1
            else:
                print(classesNow[str(trainD[i][1])])
                classesNow[str(trainD[i][1])] = classesNow[str(trainD[i][1])] + 1
    ...
    def train(self, t, trn_loader, val_loader,output_path):
        ...
        if t > 0:
            ...
            if t==0:
                adaRet_nepochs = 2*self.ret_nepochs
            else:
