...
from .sslModelsOnline import BarlowTwins
from .sslModels import SqueezeNet
import sys
...
class Appr(Learning_Appr):
    ...
    def __init__(
        ...
        self.updateBackbone = updateBackbone
        ...
        # internal vars
        self._step = 0
        self._encoder_emb_dim = featureMapSize
    ...
    def extra_parser(args):
        ...
        parser.add_argument("--expertArch", default="ResNet9", type=str, choices=['ResNet9', 'ResNet18','SqueezeNet','threLayer'])
        parser.add_argument("--mainArch", default="ResNet18", type=str, choices=['ResNet9' , 'ResNet18', 'SqueezeNet'])
        ...
        parser.add_argument("--updateBackbone", default=1, type=int, help="Update old backbone after this batch number")
        parser.add_argument("--test_every_n_steps", default=1000, type=int, help="test every")
        parser.add_argument("--featureMapSize", type=int, default=512)
    ...
    def train(self, t, trn_loader, val_loader, num_classes, omni_trn_loader, omni_val_loader, omni_tst_loader,output_path):
        ...
        self._current_task_classes_num = num_classes
        ...
        if self.sslModel == "BT":
            self.modelFB = BarlowTwins(self.projectorArc, self.batch_size, self.lambd, self.change_lr_scheduler,
                                       ...
                                       self.norm, self.mainArch,self.omni_trn_loader,self.omni_val_loader,
                                       self.val_transforms,self.omni_tst_loader,self.test_every_n_steps,output_path,self.featureMapSize)
        ...
        elif self.expertArch == 'threLayer':
            self.modelFB.expertBackbone = threeLayerCNN(self.featureMapSize)

        # wandb.watch(self.modelFB.backbone, log='all',log_freq=1)# get gradients

        self.modelFB.projectorExp = utilsProj.prediction_mlp(512, 256, self.featureMapSize, self.linearProj)
        self.modelFB.projectorRet = utilsProj.prediction_mlp(512, 256, 512,self.linearProjRet)
        ...
        self.optimExp, self.optimMain = self.modelFB.configure_Dualoptimizers(self.lrExpF, 1)
    ...
    def train_loop(self, t, trn_loader, val_loader):
        ...
        for i, ((x0, x1), _, _) in enumerate(trn_loader):

            # print(i,self.updateBackbone,i % self.updateBackbone)
            # Update the old encoder
            if (i % self.updateBackbone == 0 and i >= self.updateBackbone ):
            # if (1):
                if i > self.updateBackbone*2:
                ...
                if self.expertArch == 'ResNet18':
                    self.modelFB.copyExpert = models.resnet18(pretrained=False, zero_init_residual=True)
                    self.modelFB.copyExpert.fc = nn.Identity()
                    self.modelFB.copyExpert.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=2, bias=False)
                    self.modelFB.copyExpert.maxpool = nn.Identity()
                elif self.expertArch == 'ResNet9':
                    self.modelFB.copyExpert = ResNet9()

                elif self.expertArch == 'SqueezeNet':
                    self.modelFB.copyExpert = SqueezeNet()

                elif self.expertArch == 'threLayer':
                    self.modelFB.copyExpert = threeLayerCNN(self.featureMapSize)

                with torch.no_grad():
                    self.modelFB.copyExpert.load_state_dict(self.modelFB.expertBackbone.state_dict()) # copy weights and stuff
                ...
                if contTransfers == 0:
                    ...
                    for param in self.modelFB.backbone.parameters():
                        param.requires_grad = False
                    contTransfers += 1
                    # self.optimizer, self.scheduler = self.modelFB.configure_optimizers(self.lrExpF, 1)
                    self.optimExp, self.optimMain = self.modelFB.configure_Dualoptimizers(self.lrExpF, 1)
                else:
                    contTransfers += 1
                    flagRet = True
            ...
            if flagRet and contRetEpochs<self.ret_nepochs:
                self.modelFB.retrospection = True
                contRetEpochs += 1
                if contRetEpochs >= self.ret_nepochs:
                    contRetEpochs = 0
                    flagRet = False
                    self.modelFB.retrospection = False
                    for param in self.modelFB.backbone.parameters():
                        param.requires_grad = False
                    self.modelFB.copyBackbone = self.modelFB.backbone
                    for param in self.modelFB.copyBackbone.parameters():
                        param.requires_grad = False
                    flagTrans = True
                    self.modelFB.expertBackbone = deepcopy(self.modelFB.backbone)

            # compute output and loss
            loss = self.modelFB(x1=x0.to(self.device), x2=x1.to(self.device), batch_idx = i)

            # compute gradient and do SGD step
            self.optimExp.zero_grad()
            self.optimMain.zero_grad()
            loss.backward()
            self.optimExp.step()
            self.optimMain.step()
...
class threeLayerCNN(nn.Module):
    def __init__(self,featureMapSize):
        ...
        self.conv = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.MaxPool2d((3, 3)),
            nn.Conv2d(64,featureMapSize, (3, 3)),
        )
