...
class MemoryDataset(Dataset):
    ...
    def split(self, ratio):
        ...
        if ratio == 1.0:
            print('Subset creation with ratio 1.0 - nothing to do here!')
            return self

        selected_indices = []
        print('Sampling...')
        for lbl, lbl_count in zip(*np.unique(self.labels, return_counts=True)):
            lbl_sample = int(lbl_count * ratio)
            print(f'Class: {lbl} count: {lbl_count} -> {lbl_sample}')
            indices = np.flatnonzero(self.labels == lbl)[:lbl_sample].tolist()
            selected_indices.extend(indices)

        subset_data = {'x': [self.images[i] for i in selected_indices], 'y': [self.labels[i] for i in selected_indices]}
        subset = MemoryDataset(subset_data, self.transform, self.offset, self.class_indices)
        return subset

    def splitInbalance(self, unbalanceRation):

        #unbalanceRation = [1.0,0.5,1.0,0.5,1.0,0.5,1.0,0.5,1.0,0.3]
        countRatio = 0
        selected_indices = []
        print('Sampling...')
        for lbl, lbl_count in zip(*np.unique(self.labels, return_counts=True)):
            lbl_sample = int(lbl_count * unbalanceRation[countRatio])
            print(f'Class: {lbl} count: {lbl_count} -> {lbl_sample}',unbalanceRation[countRatio],lbl)
            countRatio += 1
            indices = np.flatnonzero(self.labels == lbl)[:lbl_sample].tolist()
            selected_indices.extend(indices)
