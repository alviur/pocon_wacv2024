...
def main(argv=None):
    ...
    for t, (_, ncla) in enumerate(taskcla):
        ...
        if args.approach in SS_APPROACHES and (args.eval_omni_head or args.eval_fresh_head or args.eval_prototypes) and t>=0:#t>1:
            ...
            #     # train fresh heads
            #     accuProto, prototypes = appr.findPrototypes(t, trn_loader[t], val_loader[t])
            #
            #     allPrototypes.append(prototypes)
            #
            #     tawProto.append(accuProto)
            #     print("Accu Prototypes task ",t," :",tawProto)
            #
            #     if(t>0):
            #         for protoT in range(t-1):
            #             driftPrototypes = appr.mapPrototypes(allPrototypes[protoT],t)
            #
            #             mappedProto[protoT] = driftPrototypes
            #         mappedProto.append(prototypes)
            #     else:
            #         mappedProto.append(prototypes)
            #
            #         # evaldriftedPrototypes
            #
            #     # eval prototypes till t
            #
            #     if (t > 0):
            #
            #         accuTillT = appr.evaldriftedPrototypes(t, mappedProto,trn_loader, val_loader)
            #
            #         accAcum.append(accuTillT)
            #
            #         print("==Drifted corrected Accuracy:", accAcum)
            ...
            if args.eval_omni_head:
                ...
                # appr.train_downstream_classifier(
                #     0, omni_trn_loader[0], omni_val_loader[0], "omni"
                # )
                ...
                omni_head = appr.train_downstream_classifier(
                    t, omni_trn_loader[0], omni_val_loader[0], "omni"
                )



                # test_loss, omni_head_acc_taw[t], omni_head_acc_tag[t], matrices_omni = appr.eval(
                ...
                test_loss, omni_head_acc_taw[t], omni_head_acc_tag[t] = appr.eval(
                    t, omni_tst_loader[0], heads_to_evaluate=[omni_head]
                )
                ...
                # wandb.log({"Omnihead accuracy main": 100 * omni_head_acc_taw[t], 'Task': t+1})

                accumain.append(100 * omni_head_acc_taw[t])
                print("Main Acc: ",accumain)
                ...
                ts = int(time.time())
                #torch.save(matrices_omni, f'{output_path}/eval_omni-task_{t}-eval_task_{_t}-time_{ts}.pb')

            # save model
            models_to_save = {
                'simsiam': appr.modelFB.state_dict()
            }
            torch.save(models_to_save, f'{output_path}/model-task_{t}.ckpt')
